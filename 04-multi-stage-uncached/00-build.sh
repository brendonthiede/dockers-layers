...
_cmd="docker build . -t ${IMAGE_NAME} --progress=plain --build-arg MESSAGE2='wish this would cache'"
printf "\n[INFO] docker command:\n\n%s\n\n" "${_cmd}"
time eval "${_cmd}"

printf "\n[INFO] Message file contents: %s\n\n" "$(docker run --rm docker-layers:04 cat message.txt)"

_cmd="docker build . -t ${IMAGE_NAME} --progress=plain --build-arg MESSAGE2='it did not cache'"
printf "\n[INFO] docker command:\n\n%s\n\n" "${_cmd}"
time eval "${_cmd}"
